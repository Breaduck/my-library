...
// 마이그레이션을 따로 돌리지 않아도 되도록 첫 호출에서 테이블을 보장한다(IF NOT EXISTS).
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  await env.DB.prepare(
    `INSERT INTO widget_data (email, token, streak, xp, level, level_title, today_pages, daily_goal, freezes, read_today, display_name, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(email) DO UPDATE SET
       ...
       read_today = excluded.read_today, display_name = excluded.display_name, updated_at = excluded.updated_at`
  ).bind(
    me, token,
    b.streak ?? 0, b.xp ?? 0, b.level ?? 1, b.levelTitle ?? '',
    b.todayPages ?? 0, b.dailyGoal ?? 30, b.freezes ?? 0, b.readToday ? 1 : 0,
    b.displayName ?? '', new Date().toISOString(),
  ).run();
