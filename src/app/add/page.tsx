...
function AddPageInner() {
  ...
  // Load URL params for prefilling (from discover page)
  useEffect(() => {
    ...
    if (paramIsbn || paramTitle) {
      fetchPages(paramIsbn ?? undefined, paramTitle ?? undefined, paramAuthor ?? undefined);
    }
  ...
  async function handleSelect(r: BookSearchResult) {
    ...
    if (r.pages) {
      setPages(String(r.pages));
    } else {
      fetchPages(r.isbn, r.title, r.author);
    }
  }
  ...
  function handleSave() {
    ...
    addBook({
      ...
      startDate, endDate, review, rating,
      totalReadingTime: 0,
    ...
    <div className="min-h-screen bg-[#F5F5F7]">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-6 sm:pt-12 pb-32">
        ...
        <div className="flex items-center gap-3 mb-8">
          <Link href="/" className="flex items-center justify-center w-11 h-11 sm:w-9 sm:h-9 rounded-full bg-white hover:bg-gray-50 active:bg-gray-100 transition-colors flex-shrink-0" style={{ boxShadow: '0 1px 8px rgba(0,0,0,0.10)' }}>
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="bg-white rounded-2xl p-5 sm:p-6" style={cs}>
            ...
            <input type="text" value={oneLiner} onChange={(e) => setOneLiner(e.target.value.slice(0, 100))}
              placeholder="예) 존재의 무게를 온몸으로 느끼게 해준 소설" className={inp} />
            ...
            <div className="bg-white rounded-2xl p-5 sm:p-6" style={cs}>
              ...
              <div className="w-1/2 pr-1.5">
                ...
                <input type="number" value={pages} onChange={(e) => setPages(e.target.value)}
                  placeholder="예) 328" className={inp} min="1" />
              </div>
              {status === 'reading' && (
                <div className="mt-3 w-1/2 pr-1.5">
                  ...
                  <input type="number" value={currentPage} onChange={e => setCurrentPage(e.target.value)}
                    placeholder="예) 145" className={inp} min="1" />
                </div>
              )}
            </div>
          )}
            ...
            <>
              ...
              <div className="bg-white rounded-2xl p-5 sm:p-6" style={cs}>
                ...
                <textarea value={review} onChange={(e) => setReview(e.target.value)}
                  placeholder="이 책을 읽고 어떤 생각이 들었나요?" rows={5} className={`${inp} resize-none`} />
              </div>
              <div className="bg-white rounded-2xl p-5 sm:p-6" style={cs}>
                    ...
                    <div key={i} className="space-y-2">
                      <div className="flex gap-2 items-start">
                        <textarea value={q.text} onChange={(e) => updateQuote(i, 'text', e.target.value)}
                          placeholder="기억에 남는 구절..." rows={3} className={`flex-1 ${inp} resize-none`} />
                        {quotes.length > 1 && (
                      ...
                      <input type="text" value={q.page} onChange={(e) => updateQuote(i, 'page', e.target.value)}
                        placeholder="p. 페이지 (선택)" className="w-36 px-3 py-2 rounded-lg bg-[#F5F5F7] text-xs text-[#1D1D1F] placeholder-[#AEAEB2] outline-none focus:ring-2 focus:ring-[#0071E3] transition-all" />
                    </div>
                  ))}
      ...
      <div className="fixed bottom-0 left-0 right-0 px-4 sm:px-6 pt-3 bg-[#F5F5F7]/90 backdrop-blur-md border-t border-black/5"
        ...
        <div className="max-w-2xl mx-auto">
          <button type="button" onClick={handleSave} disabled={!title.trim()}
            className="w-full py-4 bg-[#1D1D1F] text-white rounded-2xl text-sm font-semibold hover:bg-[#3A3A3C] active:scale-[0.98] transition-all disabled:opacity-40 disabled:cursor-not-allowed">
            저장하기
          </button>
        </div>
      </div>
          ...
          <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-6"
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="책 제목"
                  className="w-full px-4 py-3 rounded-xl bg-[#F5F5F7] text-sm text-[#1D1D1F] placeholder-[#AEAEB2] outline-none focus:ring-2 focus:ring-[#0071E3] transition-all" autoFocus />
              </div>
              <div>
                <label className="block text-xs font-medium text-[#6E6E73] mb-1.5">저자</label>
                <input type="text" value={author} onChange={(e) => setAuthor(e.target.value)} placeholder="저자 이름"
                  className="w-full px-4 py-3 rounded-xl bg-[#F5F5F7] text-sm text-[#1D1D1F] placeholder-[#AEAEB2] outline-none focus:ring-2 focus:ring-[#0071E3] transition-all" />
              </div>
              <div>
              ...
              <button type="button" onClick={() => setShowManual(false)}
                className="w-full mt-2 py-3.5 bg-[#1D1D1F] text-white rounded-xl text-sm font-semibold active:scale-[0.98] transition-all">
                확인
              </button>
            </div>
          </div>
