...
const STATUS_OPTIONS: { key: ReadingStatus; label: string; emoji: string; active: string }[] = [
  ...
  { key: 'reading', label: '읽는중',   emoji: '📖', active: 'bg-blue-500 text-white border-blue-500' },
...
const STATUS_BADGE: Record<ReadingStatus, { label: string; cls: string }> = {
  ...
  reading: { label: '읽는중',   cls: 'bg-blue-50 text-blue-600' },
            ...
            <div className="rounded-3xl overflow-hidden relative mb-3 sm:mb-4"
              ...
              <div className="relative p-5 sm:p-7">
                ...
                <textarea value={liveReview} onChange={(e) => setLiveReview(e.target.value)} onBlur={commitReview}
                  ...
                  className="w-full px-4 py-3.5 rounded-2xl text-[15px] text-[#1D1D1F] placeholder-[#AEAEB2] outline-none focus:ring-2 transition-all resize-none"
                  style={{
                    ...
                    lineHeight: 1.95,
                    ['--tw-ring-color' as never]: 'rgba(0,0,0,0.12)',
                  }}
                />
                {liveReview.length > 0 && (
                  ...
                  {liveQuotes.map((q, i) => (
                    <div key={q.id} className="relative rounded-2xl p-3.5 pl-5 space-y-2 overflow-hidden"
                      ...
                      <div className="relative flex gap-2 items-start">
                        ...
                        />
                        {liveQuotes.length > 1 && (
                          <button type="button" onClick={() => removeLiveQuote(i)} className="mt-0.5 w-7 h-7 flex items-center justify-center rounded-lg text-[#AEAEB2] hover:text-red-400 hover:bg-red-50/60 transition-colors flex-shrink-0">
                      ...
                      <input type="text" value={q.page} onChange={(e) => updateLiveQuote(i, 'page', e.target.value)} onBlur={commitQuotes}
                        ...
                        className="w-32 px-2.5 py-1.5 rounded-lg text-xs text-[#1D1D1F] placeholder-[#AEAEB2] outline-none focus:ring-2 transition-all"
                        style={{
                          background: '#F5F5F7',
                          ['--tw-ring-color' as never]: 'rgba(0,0,0,0.12)',
                        }} />
